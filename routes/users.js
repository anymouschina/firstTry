...
async function userLogin(reply,from = 0,result = []){
  if(from == '1'||from =='3'){
    ...
    let userJoinArr = []
    userJoin.map(item=>{
      userJoinArr.push(item.luckDrawId)
    })
    let list = await models.luckDraws.find({isFinish:false}).sort({created:-1}).map(item=>{
      return item.map(n=>{
        console.log(userJoinArr,n._id,userJoinArr.filter(i=>i==n._id),'!!!')
        if(userJoinArr.filter(i=>i==n._id).length>0){
          return {
            ...n._doc,
            userJoin:true
          }
...
},
{
  ...
  handler: async (request, reply) => {
    await models.users.updateMany({},{$set:{todayRegister:true,useSkinChipNum:0,skinChipNum:request.query.skinChipNum}}, function (err,res) {
      if(err)reply({status:200,data:'无此用户数据'});
      reply({status:200,data:res||'无此用户数据'});
    })
